...
//		contador=contador+1;
	...
	$scope.updatePerson = function(sessionData){
		...
		var personUpdated={"id":sessionData.id,"name": sessionData.name,"email":sessionData.email,"password":sessionData.newPassword};		
		personService.updatePerson(personUpdated).then(function(message){
			$scope.logOut2();
			personForLogin={"email":sessionData.email,"password":sessionData.newPassword};
			$scope.logIn(personForLogin);
		})
	}
}]);